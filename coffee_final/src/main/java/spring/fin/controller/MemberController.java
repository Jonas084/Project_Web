...
public class MemberController {
	...
	public void mem_idcheck(HttpServletRequest request, HttpServletResponse response) throws SQLException, IOException {
	...
	public ModelAndView zip_find_ok(HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	public void mem_join_ok(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (db_id != null) {// 중복 아이디가 있는 경우
			...
			m.setMem_zip(mem_zip);
			m.setMem_zip2(mem_zip2);
			m.setMem_addr(mem_addr);
			m.setMem_addr2(mem_addr2);
	...
	public ModelAndView login_ok(HttpServletRequest request, HttpServletResponse response, 
		...
		MemberDTO bean = this.memService.idCheck(id);
		// 기존 비번 검색
		if (bean != null) {
			if (bean.getUserPW().equals(pwd)) {
				session.setAttribute("id", id);
				session.setAttribute("name", bean.getUserName());
				session.setAttribute("valid", "Y");
				// main.jsp 페이지 이동
				/*ModelAndView mv = new ModelAndView("recommand");
				return mv;*/
				//response.sendRedirect("recommand.do");
				out.println("<script>");
	...
	public ModelAndView pwd_find_ok(HttpServletRequest request, HttpServletResponse response) throws IOException {
		response.setContentType("text/html;charset=UTF-8");
		PrintWriter out = response.getWriter();
		...
		SpringMemberBean bean1 = this.memService.findPwd(bean);
		if (bean1 != null) {
			ModelAndView mv = new ModelAndView("member/pwd_find_ok");
	...
	public ModelAndView mem_edit(HttpServletRequest request, HttpServletResponse response, 
	...
	public ModelAndView mem_edit_ok(HttpServletRequest request, HttpServletResponse response, 
																HttpSession session) 	throws Exception {
		response.setContentType("text/html;charset=UTF-8");
		PrintWriter out = response.getWriter();
		session = request.getSession();
		...
		if (id == null) {
			...
			String mem_phone02 = request.getParameter("mem_phone02").trim();
			String mem_phone03 = request.getParameter("mem_phone03").trim();
			...
			m33.setMem_id(id);
			m33.setMem_pwd(mem_pwd);
			m33.setMem_name(mem_name);
			m33.setMem_zip(mem_zip);
			m33.setMem_zip2(mem_zip2);
			m33.setMem_addr(mem_addr);
			m33.setMem_addr2(mem_addr2);
			m33.setMem_phone(mem_phone);
	...
	public ModelAndView mem_DEL(HttpServletRequest request, HttpServletResponse response, 
															HttpSession session) throws Exception {
		response.setContentType("text/html;charset=UTF-8");
		PrintWriter out = response.getWriter();
		session = request.getSession();
	...
	public ModelAndView mem_del_ok(HttpServletRequest request, HttpServletResponse response, 
																	HttpSession session) 	throws Exception {
		response.setContentType("text/html;charset=UTF-8");
		// 웹상에 보이는 언어코딩 타입 지정
